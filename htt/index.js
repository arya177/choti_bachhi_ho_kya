...
const initial = (arr, projectName, page, index) => {
  ...
  for (let x in arr) {
    const dt = fs.readFileSync(divStylefiletop, 'utf-8', { flag: 'r' });
    ...
    if (Array.isArray(arr[x]["children"]) === "true") {
      ...
      for (let y in arr[x]["children"]) {
        const dst = fs.readFileSync(divfiletop, 'utf-8', { flag: 'r' });
        fs.appendFileSync(copyto, dst, { flag: "a+" });
        ...
        const pst = fs.readFileSync(divfilebottom, 'utf-8', { flag: 'r' });
        fs.appendFileSync(copyto, pst, { flag: "a+" });
      }
      const dt = fs.readFileSync(divStylefilebottom, 'utf-8', { flag: 'r' });
      fs.appendFileSync(copyto, dt, { flag: "a+" });
    }
    if (arr[x].type === "column") {
      const dst = fs.readFileSync(divfilebottom, 'utf-8', { flag: 'r' });
      fs.appendFileSync(copyto, dst, { flag: "a+" });
    } if (arr[x].type === "row") {
      const dsb = fs.readFileSync(divStylefilebottom, 'utf-8', { flag: 'r' });
      fs.appendFileSync(copyto, dsb, { flag: "a+" });
    }
  }
...
const process = async (projectName) => {
  ...
  queue.enqueue(home);
  queue.enqueue(new file("Blogs", projectDirectory));
  queue.enqueue(new file("About", projectDirectory));
